...
#player 1 draws letters from the bag of letters so that they have 7 letters 
def choose_letters_from_bag_player_1(player_1_letters, letter_bag):
    ...
    return player_1_letters, letter_bag

#player 2 draws letters from the bag of letters so that they have 7 letters 
def choose_letters_from_bag_player_2(player_2_letters, letter_bag):
    ...
    return player_2_letters, letter_bag
...
def exchange_player_1_letters(ans, player_1_letters, letter_bag):
    ...
    #if no letters left in bag 
    if len(letter_bag) == 0:
        print("\nThere are no letters left in the bag! Try and make a word with the letters you have.\n")
        return 0
    #if trying to exchange too many letters
    elif len(ans)-1 > 7:
...
#taunts after player 1's turn 
def taunt_player_1(points):
    os.system("clear")
    bad_taunt = random.randint(1, 3)
    good_taunt = random.randint(1, 2)
    if points <= 5:
        if bad_taunt == 1:
            print("\n\nHmmmm, you're not very good at this, are you? You only get {0} points for {1}.".format(points, ', '.join(new_words)))
        elif bad_taunt == 2:
            print("\n\nGosh, is that the best you can do? You only get {0} points for {1}.".format(points, ', '.join(new_words)))
        else:
            print("\n\nI could play that in my sleep! You only get {0} points for {1}.".format(points, ', '.join(new_words)))
    elif points < 15:
        if bad_taunt == 1:
            print("\n\nEh, that's a pretty average move. You get {0} points for {1}.".format(points, ', '.join(new_words)))
        elif bad_taunt == 2:
            print("\n\nIf you're trying to impress me, you're not doing so well. You get {0} points for {1}.".format(points, ', '.join(new_words)))
        else:
            print("\n\nCome on, you can do better than that! You get {0} points for {1}.".format(points, ', '.join(new_words)))
    elif points < 30:
        if good_taunt == 1:
            print("\n\nOkay, I've got to admit, that's a pretty good move. You get {0} points for {1}.".format(points, ', '.join(new_words)))
        else:
            print("\n\nNot bad! I see you're learning something from me. You get {0} points for {1}.".format(points, ', '.join(new_words)))
    elif points < 50:
        if good_taunt == 1:
            print("\n\nUgh, you're actually really good at this. You get {0} points for {1}.".format(points, ', '.join(new_words)))
        else:
            print("\n\nOkay, I'm starting to get a little worried now. You get {0} points for {1}.".format(points, ', '.join(new_words)))
    elif points >= 50:
        print("\n\nWOW! You're not supposed to be this good, you're a human! You get a whopping {0} points for {1}.".format(points, ', '.join(new_words)))

#taunts after player 2's turn 
def taunt_player_2(points):
    os.system("clear")
    bad_taunt = random.randint(1, 4)
    good_taunt = random.randint(1, 2)
    if points <= 5:
        if good_taunt == 1:
            print("\n\nWell, this is embarrassing. I only get {0} points for {1}.".format(points, ', '.join(new_words)))
        else:
            print("\n\nUghhhh, I'm not used to failure. I only get {0} points for {1}.".format(points, ', '.join(new_words)))
    elif points < 15:
        if good_taunt == 1:
            print("\n\nOops, I couldn't find anything better. Don't judge me. I only get {0} points for {1}.".format(points, ', '.join(new_words)))
        else:
            print("\n\nCan you please not tell anyone about how I only got {0} points for {1}?".format(points, ', '.join(new_words)))
    elif points < 30:
        if bad_taunt == 1:
            print("\n\nHmmmm, I'm moderately proud of myself. I get {0} points for {1}.".format(points, ', '.join(new_words)))
        elif bad_taunt == 2:
            print("\n\nOkay, that was good, but I'm clearly not living up to my full potential. I get {0} points for {1}.".format(points, ', '.join(new_words)))
        elif bad_taunt == 3:
            print("\n\nYou think that was good? Just you wait. I get {0} points for {1}.".format(points, ', '.join(new_words)))
        else:
            print("\n\nYou know what, I'm pretty happy with that. I get {0} points for {1}.".format(points, ', '.join(new_words)))
    elif points < 50:
        if good_taunt == 1:
            print("\n\nDon't you wish you were as clever as me? I get {0} points for {1}.".format(points, ', '.join(new_words)))
        else:
            print("\n\nYep, I'm just that good. Don't be jealous. I get {0} points for {1}.".format(points, ', '.join(new_words)))
    elif points >= 50:
...
def find_possible_words(row, player_2_letters, scrabble_words):
    ...
    #check if the set of letters in the word is a subset of the set of possible letters we can use (i.e. the letters in that row/col)
...
if play == 'n':
    ...
    player_1_letters = []
    player_2_letters = []
    player_1_letters, letter_bag = choose_letters_from_bag_player_1(player_1_letters, letter_bag)
    player_2_letters, letter_bag = choose_letters_from_bag_player_2(player_2_letters, letter_bag)
    ...
    #load all allowed scrabble words and store in a dictionary (hash table) for fast lookup 
    ...
    while letter_bag != [] or (letter_bag == [] and player_1_letters != [] and player_2_letters != []):
        ...
        while valid == 0:
            ...
            ans = input("Enter each letter followed by the row and column where you want to play it (eg. A 1 2, B 13 4), or Exch followed by the letters you want to exchange (eg. Exch A B), or Skip if you want to skip your turn :\n")
            ...
            #player 1 takes new letters from bag 
            player_1_letters, letter_bag = choose_letters_from_bag_player_1(player_1_letters, letter_bag)
            
            printboard(board, letter_bonus, word_bonus)

            #if player 1 has used up all their letters, game over!
            if len(player_1_letters) == 0:
                game_over = 1 

        #check if game over (if player 1 has used up all their letters)
        if game_over == 1:
            break
        ...
        while valid == 0:
            ...
            if best_word == []:
                #if there are letters left in the bag, exchange 
                if len(letter_bag) != 0:
                    exchange_player_2_letters(player_2_letters, letter_bag)
                    break 
                else: #if there are no letters left in the bag, skip 
                    print("\nI'm skipping my turn! The letter bag is empty and I can't think of any words.\n")
                    break
